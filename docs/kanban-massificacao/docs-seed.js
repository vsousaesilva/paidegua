/**
 * pAIdegua Kanban — Seed dos Documentos compartilhados
 *
 * Pacote inicial de manuais, dicas, pipelines e runbooks. Carregado pelo botão
 * "📦 Carregar pacote inicial" do modal Documentos quando a base está vazia.
 *
 * NÃO é cifrado — Documentos são compartilhados em texto plano para a equipe
 * Inovajus. Conteúdo sensível (chaves, senhas) deve ir no Cofre (vault), não aqui.
 *
 * Após o carregamento, os 11 itens ficam em KV (vault:state → docs:state) e
 * o seed pode ser desativado removendo o `<script src="docs-seed.js">` do index.html.
 */
window.__PAIDEGUA_DOCS_SEED__ = [
  {
    titulo: '📋 Bem-vindo aos Documentos do projeto',
    tags: ['indice', 'leia-primeiro'],
    conteudo: `# 📋 Documentos do pAIdegua

Bem-vindo. Este espaço reúne **manuais, runbooks, pipelines e dicas** do projeto
pAIdegua, **compartilhados na equipe Inovajus**. Tudo aqui é texto plano
versionado em KV — **não coloque senhas ou chaves aqui**, use o **🔐 Cofre**.

## O que está aqui

- **Manual de massificação** — roadmap F1-F7
...
- **Reuniões e ritos do projeto** — daily, planning, retro, comitê
- **Política de privacidade pAIdegua** — espelho de docs/index.md

## Convenções

- **Markdown puro**. Sem extensões. Tabelas, listas, código com crase tripla, links normais.
- **Tags** ajudam a buscar. Use kebab-case minúsculo (\`pipeline\`, \`anti-padroes\`, \`pje\`).
- **Quem edita assina**: o histórico do KV registra quem foi o último a alterar
  (mostrado no rodapé do editor).
- **Histórico**: o KV mantém a versão atual; para histórico de revisões, use o
  Git do repo \`vsousaesilva/paidegua\` (cada deploy do Pages é um commit).

## O que NÃO colocar aqui

- ❌ Senhas, API keys, tokens, certificados → vão no **🔐 Cofre**
- ❌ Conteúdo de processos judiciais (CPF, partes) → fora do escopo desta ferramenta
- ❌ Notas pessoais de servidores → use o seu próprio Drive

## Backup

\`\`\`cmd
wrangler kv:key get --binding=KANBAN_KV "docs:state" > backup-docs-%date:~6,4%-%date:~3,2%-%date:~0,2%.json
\`\`\`
`,
  },
  {
    titulo: 'Manual de Massificação do pAIdegua (resumo)',
    tags: ['manual', 'roadmap', 'massificacao', 'f1-f7'],
    conteudo: `# Manual de Massificação do pAIdegua — resumo

> Documento original (autoridade): \`docs/manual-massificacao-pje.md\` no repo paidegua.
...
`,
  ...
  {
    titulo: 'Boas práticas de comunicação com o PJe',
...
`,
  ...
  {
    titulo: '⚠️ Anti-padrões — o que NÃO fazer',
...
`,
  ...
  {
    titulo: 'Pipeline — Build da extensão pAIdegua',
...
`,
  ...
  {
    titulo: 'Pipeline — Release e versionamento',
...
`,
  ...
  {
    titulo: 'Pipeline — Deploy do Kanban (kanban.paidegua.ia.br)',
...
### Front (Pages)
\`\`\`cmd
cd /d "C:\\Users\\vsousaesilva\\...\\paidegua"
git add docs/kanban-massificacao/
git commit -m "ajuste no kanban"
git push
wrangler pages deploy "docs/kanban-massificacao" --project-name=paidegua-kanban --branch=main --commit-dirty=true
\`\`\`

### Worker (manual)
\`\`\`cmd
cd /d "C:\\Users\\vsousaesilva\\...\\paidegua\\docs\\kanban-massificacao\\worker"
wrangler deploy
\`\`\`

### Bumpar versão de cache (front)
Editar \`index.html\`: trocar \`?v=N\` para \`?v=N+1\` em \`kanban.css\`, \`kanban.js\`,
\`vault.js\`, \`docs.js\`. Garante reload em quem já tem aba aberta.

## Adicionar membro à equipe

**Caminho A — wrangler.toml**: editar \`ALLOWED_EMAILS\`:
\`\`\`toml
ALLOWED_EMAILS = "vsousaesilva@jfce.jus.br,ana@jfce.jus.br"
\`\`\`
Depois \`wrangler deploy\` no diretório \`worker/\`.

**Caminho B — sem redeploy** (interface Cloudflare): Workers & Pages →
...
|---|---|---|
| OTP não chega | Resend → Logs | Verificar SPF/DKIM no DNS |
| 401 nas APIs | Token expirado (>90d) | Re-login |
| Issue não cria no GH | Token GH expirado | Renovar PAT |
| Card não move | Modal aberto | Fechar modal antes |

## Backup

\`\`\`cmd
wrangler kv:key get --binding=KANBAN_KV "board:state" > backup-board.json
wrangler kv:key get --binding=KANBAN_KV "docs:state" > backup-docs.json
wrangler kv:key get --binding=KANBAN_KV "vault:state" > backup-vault.json
\`\`\`
`,
  },
  {
    titulo: 'Manual de Comandos do Kanban (modo A) — cheatsheet',
...
`,
  ...
  {
    titulo: 'Onboarding — novo membro Inovajus',
...
### Repositórios
- [ ] Acesso de leitura ao \`vsousaesilva/paidegua\`
...
- [ ] Documentos → "Manual de Massificação"
- [ ] Documentos → "Boas práticas de comunicação com o PJe"
- [ ] Documentos → "Anti-padrões"
- [ ] Documentos → "Política de privacidade pAIdegua"
- [ ] \`docs/manual-instalacao-uso.md\` no repo paidegua

## Semana 1 — leituras recomendadas

- [ ] \`docs/extracao-conteudo-pje.md\`
- [ ] \`docs/post-mortem-prazos-na-fita.md\`
- [ ] \`docs/telemetria-local-e-escala.md\`
- [ ] Documentos → "Manual de comandos do Kanban"
...
`,
  ...
  {
    titulo: 'Reuniões e ritos do projeto',
...
`,
  ...
  {
    titulo: 'Política de Privacidade pAIdegua (resumo)',
