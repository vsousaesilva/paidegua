...
export function buildAnaliseProcessoPrompt(
  ...
  return (
    ...
    `- Se um documento aparecer em "DATAS CANDIDATAS" com "nenhuma data encontrada", considere o critério correspondente como NÃO atendido e solicite novo documento com data legível. EXCEÇÃO: para procuração com rogo estrutural presente (ver regras adiante), trate como atendido e recomende apenas a conferência visual da data no PDF original, pois templates com preenchimento manuscrito frequentemente escapam ao OCR automático.\n\n` +
    `REGRAS ESPECÍFICAS PARA DATAS:\n` +
    `- Procuração: a data relevante é a da assinatura do outorgante, tipicamente no final do documento, imediatamente antes ou depois do bloco de assinatura, no padrão "Cidade/UF, DD de mês de AAAA" ou "Cidade/UF, DD/MM/AAAA". Quando houver várias datas candidatas, escolha a que está ao lado de um topônimo brasileiro (cidade/UF) e, havendo empate, a mais recente/posterior no texto. JAMAIS use datas que apareçam dentro do corpo dos poderes citando normas, portarias, leis, decretos, resoluções, instruções normativas, artigos, súmulas, acórdãos, ou datas de documentos pessoais (RG, CPF, nascimento) — essas são apenas citadas, não são a data de emissão. Se NENHUMA data candidata for plausível (todas são citações de normas/docs pessoais ou a lista é vazia) mas a procuração tiver estrutura de rogo identificável, NÃO rejeite pelo prazo: mencione na justificativa que "a data de assinatura não foi extraída automaticamente pelo OCR; verificar no PDF original" e mantenha o critério como atendido. Se houver substabelecimento, a procuração de referência é a do outorgante originário, não a do substabelecimento.\n` +
    ...
    `- Considere o critério ATENDIDO sob a forma a rogo quando houver, no texto extraído da procuração, QUALQUER evidência dos seguintes elementos: (a) marcador de rogo — qualquer ocorrência de termos como "rogo", "a rogo", "assina a rogo", "assina o rogado", "ragado" (erro comum de digitação), "rogante", "a pedido de", "por não saber/poder assinar", "declaro-me analfabeto(a)", ou equivalentes, mesmo isolados ou em caixa alta dentro de template impresso; (b) presença de rogado — nome/linha destinada a quem assina pelo outorgante; (c) duas testemunhas — qualquer ocorrência de duas rotulações "Testemunha", "Test.:" ou pares de "CPF:"/"RG:" no bloco de assinaturas.\n` +
    `- TOLERÂNCIA AO OCR: procurações em papel são frequentemente templates impressos com preenchimento manuscrito — o OCR costuma capturar o template (ex.: "A ROGO do(a) TITULAR, Assina o Ragado:", "Testemunha:", "CPF:") mas falhar nos nomes e CPFs escritos à mão. Quando o TEMPLATE contiver marcadores de rogo e rótulos de testemunhas mas os preenchimentos forem ilegíveis, PRESUMA o critério atendido (o examinador humano conferirá no PDF) e mencione na justificativa que a verificação dos nomes/CPFs manuscritos depende da conferência visual do original. Não rejeite pela simples falta de texto OCR nos campos manuscritos.\n` +
    `- Variações ortográficas e erros de digitação comuns ("Ragado", "Rogante", "arrogo") NÃO descaracterizam a procuração a rogo — valorize o sentido sobre a forma.\n` +
    `- Só marque como NÃO atendido se NÃO houver NENHUM marcador de rogo (item "a") no texto extraído, OU quando houver apenas UMA testemunha evidente. NÃO rejeite pela ausência de cópias dos documentos pessoais de rogado/testemunhas — se os três elementos estruturais estiverem presentes, considere atendido e, se desejar, mencione a juntada das cópias como recomendação, não como obstáculo.\n\n` +
    `REGRAS DE AVALIAÇÃO:\n` +
